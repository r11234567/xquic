...
struct xqc_stream_s {
    ...
    xqc_stream_write_buff_list_t stream_write_buff_list; /* buffer list for 0RTT */
    xqc_list_head_t write_stream_list, read_stream_list, closing_stream_list,
        all_stream_list;
    ...
    struct {
        ...
        xqc_int_t fec_blk_lack_num;   /* number of lack source symbol when receive last
                                         repair symbol */
        xqc_usec_t fec_blk_lack_time; /* (first block) block finish time - last received
                                         rpr (in block) time */
        xqc_usec_t
            recv_time_with_fec; /* stream received time with fec recovered packets */
...
xqc_stream_t *xqc_create_stream_with_conn(xqc_connection_t *conn,
                                          xqc_stream_id_t stream_id,
                                          xqc_stream_type_t stream_type,
...
xqc_stream_t *xqc_find_stream_by_id(xqc_stream_id_t stream_id,
...
int xqc_stream_do_create_flow_ctl(xqc_connection_t *conn, xqc_stream_id_t stream_id,
                                  xqc_stream_type_t stream_type);

xqc_stream_t *xqc_passive_create_stream(xqc_connection_t *conn, xqc_stream_id_t stream_id,
                                        void *user_data);

xqc_stream_t *xqc_create_crypto_stream(xqc_connection_t *conn,
...
ssize_t xqc_stream_buff_data(xqc_stream_t *stream, unsigned char *send_data,
...
void xqc_stream_set_multipath_usage(xqc_stream_t *stream, uint8_t schedule,
                                    uint8_t reinject);
...
xqc_stream_path_metrics_get_or_grow(xqc_stream_t *s, uint64_t path_id)
{
    ...
    if (s->paths_info_count == s->paths_info_capacity) {
        ...
        void *nb = xqc_realloc(s->paths_info, (size_t)newcap * sizeof(*s->paths_info));
        if (!nb) {
            return NULL;
        }
        s->paths_info = nb;
