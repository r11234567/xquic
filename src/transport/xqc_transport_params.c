                                 ...
                                 xqc_transport_params_type_t exttype)
{
    ...
    if (params->initial_max_stream_data_bidi_local) {
        len +=
            xqc_put_varint_len(XQC_TRANSPORT_PARAM_INITIAL_MAX_STREAM_DATA_BIDI_LOCAL) +
            xqc_put_varint_len(
    ...
    if (params->initial_max_stream_data_bidi_remote) {
        len +=
            xqc_put_varint_len(XQC_TRANSPORT_PARAM_INITIAL_MAX_STREAM_DATA_BIDI_REMOTE) +
            xqc_put_varint_len(
    ...
    if (params->initial_max_stream_data_uni) {
        len +=
            xqc_put_varint_len(XQC_TRANSPORT_PARAM_INITIAL_MAX_STREAM_DATA_UNI) +
    ...
    if (params->disable_active_migration) {
        len += xqc_put_varint_len(XQC_TRANSPORT_PARAM_DISABLE_ACTIVE_MIGRATION) +
               xqc_put_varint_len(
                   0); /* disable_active_migration is zero-length transport parameter */
    }

    /* PREFERRED_ADDRESS */
    if (exttype == XQC_TP_TYPE_ENCRYPTED_EXTENSIONS &&
        params->preferred_address_present && params->preferred_address.cid.cid_len > 0) {
    ...
    if (params->active_connection_id_limit != XQC_DEFAULT_ACTIVE_CONNECTION_ID_LIMIT) {
        len +=
            xqc_put_varint_len(XQC_TRANSPORT_PARAM_ACTIVE_CONNECTION_ID_LIMIT) +
    ...
    if (params->enable_multipath) {
        if (params->multipath_version == XQC_MULTIPATH_3E) {
            len += xqc_put_varint_len(XQC_TRANSPORT_PARAM_INIT_MAX_PATH_ID_V21) +
                   xqc_put_varint_len(xqc_put_varint_len(params->init_max_path_id)) +
                   xqc_put_varint_len(params->init_max_path_id);

        } else if (params->multipath_version == XQC_MULTIPATH_10) {
    ...
    len += xqc_put_varint_len(XQC_TRANSPORT_PARAM_FEC_VERSION_02) + xqc_put_varint_len(0);

    /*
     * if enable_encode_fec, add fec related params' length:
     * max_symbol_size; max_src_symbol_len; max_encoder_schemes;
     */
    if (params->enable_encode_fec && params->fec_encoder_schemes_num > 0 &&
     ...
     * max_decoder_schemes;
     */
    if (params->enable_decode_fec && params->fec_decoder_schemes_num > 0 &&
    ...
    if (params->extended_ack_features) {
        ...
        if (params->max_receive_timestamps_per_ack) {
            len +=
                xqc_put_varint_len(XQC_TRANSPORT_PARAM_MAX_RECEIVE_TIMESTAMPS_PER_ACK) +
                xqc_put_varint_len(
...
xqc_encode_transport_params(const xqc_transport_params_t *params,
                            xqc_transport_params_type_t exttype, uint8_t *out,
                            size_t out_cap, size_t *out_len)
{
    ...
    if (params->initial_max_stream_data_bidi_local) {
        p = xqc_put_varint_param(p,
                                 XQC_TRANSPORT_PARAM_INITIAL_MAX_STREAM_DATA_BIDI_LOCAL,
                                 params->initial_max_stream_data_bidi_local);
    }

    if (params->initial_max_stream_data_bidi_remote) {
        p = xqc_put_varint_param(p,
                                 XQC_TRANSPORT_PARAM_INITIAL_MAX_STREAM_DATA_BIDI_REMOTE,
        ...
        params->preferred_address.cid.cid_len > 0) /* cid MUST NOT be zero-length */
    {
        ...
        *p++ = params->preferred_address.cid.cid_len;
        p = xqc_cpymem(p, params->preferred_address.cid.cid_buf,
                       params->preferred_address.cid.cid_len);
        p = xqc_cpymem(p, params->preferred_address.stateless_reset_token,
                       XQC_STATELESS_RESET_TOKENLEN);
    }
    ...
    if (params->initial_source_connection_id_present) {
        ...
        p = xqc_cpymem(p, params->initial_source_connection_id.cid_buf,
                       params->initial_source_connection_id.cid_len);
    }
    ...
    if (params->retry_source_connection_id_present) {
        ...
        p = xqc_cpymem(p, params->retry_source_connection_id.cid_buf,
                       params->retry_source_connection_id.cid_len);
    }
    ...
    if (params->no_crypto) {
        p = xqc_put_varint_param(p, XQC_TRANSPORT_PARAM_NO_CRYPTO, params->no_crypto);
    }

    if (params->enable_multipath) {
        if (params->multipath_version == XQC_MULTIPATH_3E) {
            p = xqc_put_varint_param(p, XQC_TRANSPORT_PARAM_INIT_MAX_PATH_ID_V21,
                                     params->init_max_path_id);

        } else if (params->multipath_version == XQC_MULTIPATH_10) {
            p = xqc_put_varint_param(p, XQC_TRANSPORT_PARAM_INIT_MAX_PATH_ID_V10,
                                     params->init_max_path_id);
        }
    }

    if (params->enable_pmtud) {
        p = xqc_put_varint_param(p, XQC_TRANSPORT_PARAM_PMTUD_OPTIONS,
                                 params->enable_pmtud);
    }
    ...
    if (params->enable_decode_fec && params->fec_decoder_schemes_num > 0 &&
        params->fec_decoder_schemes_num <= XQC_FEC_MAX_SCHEME_NUM) {
        preferred_fec_paramslen = 0;
        if (params->fec_decoder_schemes_num > 0 &&
            params->fec_encoder_schemes_num <= XQC_FEC_MAX_SCHEME_NUM) {
            p = xqc_put_varint(p, XQC_TRANSPORT_PARAM_FEC_DECODER_SCHEMES);
            preferred_fec_paramslen = xqc_put_varint_len(params->fec_decoder_schemes_num);
            for (xqc_int_t i = 0; i < params->fec_decoder_schemes_num; i++) {
                preferred_fec_paramslen +=
                    xqc_put_varint_len(params->fec_decoder_schemes[i]);
            }
            p = xqc_put_varint(p, preferred_fec_paramslen);
    ...
    if (params->extended_ack_features) {
        ...
        if (params->max_receive_timestamps_per_ack) {
            p = xqc_put_varint_param(p,
                                     XQC_TRANSPORT_PARAM_MAX_RECEIVE_TIMESTAMPS_PER_ACK,
...
static xqc_int_t
xqc_decode_original_dest_cid(xqc_transport_params_t *params,
...
static xqc_int_t
xqc_decode_stateless_token(xqc_transport_params_t *params,
                           xqc_transport_params_type_t exttype, const uint8_t *p,
                           const uint8_t *end, uint64_t param_type, uint64_t param_len)
{
    if (exttype != XQC_TP_TYPE_ENCRYPTED_EXTENSIONS) {
...
static xqc_int_t
xqc_decode_disable_active_migration(xqc_transport_params_t *params,
                                    xqc_transport_params_type_t exttype, const uint8_t *p,
                                    const uint8_t *end, uint64_t param_type,
                                    uint64_t param_len)
{
...
static xqc_int_t
xqc_decode_preferred_address(xqc_transport_params_t *params,
                             xqc_transport_params_type_t exttype, const uint8_t *p,
                             const uint8_t *end, uint64_t param_type, uint64_t param_len)
{
    if (exttype != XQC_TP_TYPE_ENCRYPTED_EXTENSIONS) {
...
static xqc_int_t
xqc_decode_active_cid_limit(xqc_transport_params_t *params,
                            xqc_transport_params_type_t exttype, const uint8_t *p,
                            const uint8_t *end, uint64_t param_type, uint64_t param_len)
{
...
static xqc_int_t
xqc_decode_initial_scid(xqc_transport_params_t *params,
                        xqc_transport_params_type_t exttype, const uint8_t *p,
                        const uint8_t *end, uint64_t param_type, uint64_t param_len)
{
    if (param_len > XQC_MAX_CID_LEN) {
...
static xqc_int_t
xqc_decode_enable_multipath(xqc_transport_params_t *params,
                            xqc_transport_params_type_t exttype, const uint8_t *p,
                            const uint8_t *end, uint64_t param_type, uint64_t param_len)
{
    if (param_type == XQC_TRANSPORT_PARAM_INIT_MAX_PATH_ID_V10 ||
        ...
        params->multipath_version =
            (param_type == XQC_TRANSPORT_PARAM_INIT_MAX_PATH_ID_V21) ? XQC_MULTIPATH_3E
                                                                     : XQC_MULTIPATH_10;
...
static xqc_int_t
xqc_decode_enable_pmtud(xqc_transport_params_t *params,
                        xqc_transport_params_type_t exttype, const uint8_t *p,
                        const uint8_t *end, uint64_t param_type, uint64_t param_len)
{
...
static xqc_int_t
xqc_decode_close_dgram_redundancy(xqc_transport_params_t *params,
                                  xqc_transport_params_type_t exttype, const uint8_t *p,
                                  const uint8_t *end, uint64_t param_type,
                                  uint64_t param_len)
{
    ...
    switch (ret) {
    case XQC_RED_SET_CLOSE: params->close_dgram_redundancy = XQC_RED_SET_CLOSE; break;

    default: params->close_dgram_redundancy = XQC_RED_NOT_USE; break;
    }
    return XQC_OK;
}
...
#ifdef XQC_ENABLE_FEC
static xqc_int_t
xqc_decode_fec_version(xqc_transport_params_t *params,
                       xqc_transport_params_type_t exttype, const uint8_t *p,
                       const uint8_t *end, uint64_t param_type, uint64_t param_len)
{
    switch (param_type) {
    case XQC_TRANSPORT_PARAM_FEC_VERSION_02: params->fec_version = XQC_FEC_02; break;

    default: params->fec_version = XQC_ERR_FEC_VERSION; break;
    }
...
static xqc_int_t
xqc_decode_fec_max_symbols_num(xqc_transport_params_t *params,
                               xqc_transport_params_type_t exttype, const uint8_t *p,
                               const uint8_t *end, uint64_t param_type,
                               uint64_t param_len)
{
...
static xqc_int_t
xqc_decode_encoder_schemes(xqc_transport_params_t *params,
                           xqc_transport_params_type_t exttype, const uint8_t *p,
                           const uint8_t *end, uint64_t param_type, uint64_t param_len)
{
    int vlen;
    ...
    for (i = 0; i < schemes_len; i++) {
        ...
        if (xqc_set_fec_scheme(curr_scheme, &params->fec_encoder_schemes[tmp_len]) ==
            XQC_OK) {
            tmp_len++;
        }
        p += vlen;
...
static xqc_int_t
xqc_decode_decoder_schemes(xqc_transport_params_t *params,
                           xqc_transport_params_type_t exttype, const uint8_t *p,
                           const uint8_t *end, uint64_t param_type, uint64_t param_len)
{
    int vlen;
    ...
    for (i = 0; i < schemes_len; i++) {
        vlen = xqc_vint_read(p, end, &curr_scheme);
        if (xqc_set_fec_scheme(curr_scheme, &params->fec_decoder_schemes[tmp_len]) ==
            XQC_OK) {
            tmp_len++;
        }
        p += vlen;
...
static xqc_int_t
xqc_decode_extended_ack_features(xqc_transport_params_t *params,
                                 xqc_transport_params_type_t exttype, const uint8_t *p,
                                 const uint8_t *end, uint64_t param_type,
                                 uint64_t param_len)
{
...
static xqc_int_t
xqc_decode_max_receive_timestamps_per_ack(xqc_transport_params_t *params,
                                          xqc_transport_params_type_t exttype,
                                          const uint8_t *p, const uint8_t *end,
                                          uint64_t param_type, uint64_t param_len)
{
    ...
    /* [TRANSPORT] Values above 20 are invalid */
    if (nread < 0 ||
        params->max_receive_timestamps_per_ack > XQC_MAX_RECEIVE_TIMESTAMPS_PER_ACK) {
        return -XQC_TLS_MALFORMED_TRANSPORT_PARAM;
    }
    return XQC_OK;
}
...
static xqc_int_t
xqc_decode_receive_timestamps_exponent(xqc_transport_params_t *params,
                                       xqc_transport_params_type_t exttype,
                                       const uint8_t *p, const uint8_t *end,
                                       uint64_t param_type, uint64_t param_len)
{
    ...
    /* [TRANSPORT] Values above 20 are invalid */
    if (nread < 0 ||
        params->receive_timestamps_exponent > XQC_MAX_RECEIVE_TIMESTAMPS_EXPONENT) {
        return -XQC_TLS_MALFORMED_TRANSPORT_PARAM;
    }
    return XQC_OK;
}
...
static xqc_int_t
xqc_decode_max_datagram_frame_size(xqc_transport_params_t *params,
                                   xqc_transport_params_type_t exttype, const uint8_t *p,
                                   const uint8_t *end, uint64_t param_type,
                                   uint64_t param_len)
{
...
typedef enum {
    ...
    XQC_TP_DECODER_DISABLE_ACTIVE_MIGRATION,
    XQC_TP_DECODER_PREFERRED_ADDRESS,
...
typedef xqc_int_t (*xqc_trans_param_decode_func)(xqc_transport_params_t *params,
                                                 xqc_transport_params_type_t exttype,
                                                 const uint8_t *p, const uint8_t *end,
                                                 uint64_t param_type, uint64_t param_len);
...
xqc_trans_param_get_index(uint64_t param_type)
{
    ...
    case XQC_TRANSPORT_PARAM_INIT_MAX_PATH_ID_V21: return XQC_TP_DECODER_ENABLE_MULTIPATH;
    ...
    case XQC_TRANSPORT_PARAM_FEC_VERSION_02: return XQC_TP_DECODER_FEC_VERSION_PARSER;
    ...
    case XQC_TRANSPORT_PARAM_NO_CRYPTO: return XQC_TP_DECODER_NO_CRYPTO;

    case XQC_TRANSPORT_PARAM_PMTUD_OPTIONS: return XQC_TP_DECODER_PMTUD_OPTIONS;
...
xqc_decode_one_transport_param(xqc_transport_params_t *params,
                               xqc_transport_params_type_t exttype, const uint8_t **start,
                               const uint8_t *end)
{
    ...
    if (param_index != XQC_TP_DECODER_UNKNOWN) {
        xqc_int_t ret = xqc_trans_param_decode_func_list[param_index](
            params, exttype, p, end, param_type, param_len);
        if (ret < 0) {
            return -XQC_TLS_MALFORMED_TRANSPORT_PARAM;
        }
    }
...
xqc_decode_transport_params(xqc_transport_params_t *params,
                            xqc_transport_params_type_t exttype, const uint8_t *in,
                            size_t in_len)
{
    const uint8_t *p, *end;
...
xqc_int_t
xqc_read_transport_params(char *tp_data, size_t tp_data_len,
                          xqc_transport_params_t *params)
{
    ...
    while (*p != '\0' && p < e) {
        ...
        if (strncmp(p, "initial_max_streams_bidi=",
            ...
            params->initial_max_stream_data_bidi_local = strtoul(p, NULL, XQC_DECIMAL);
            ...
            params->initial_max_stream_data_bidi_remote = strtoul(p, NULL, XQC_DECIMAL);
            ...
            params->initial_max_stream_data_uni = strtoul(p, NULL, XQC_DECIMAL);

        } else if (strncmp(p, "initial_max_data=", xqc_lengthof("initial_max_data=")) ==
                   0) {
            p += xqc_lengthof("initial_max_data=");
            params->initial_max_data = strtoul(p, NULL, XQC_DECIMAL);

        } else if (strncmp(p, "max_ack_delay=", xqc_lengthof("max_ack_delay=")) == 0) {
            p += xqc_lengthof("max_ack_delay=");
            params->max_ack_delay = strtoul(p, NULL, XQC_DECIMAL);

        } else if (strncmp(p, "max_datagram_frame_size=",
...
xqc_write_transport_params(char *tp_buf, size_t cap, const xqc_transport_params_t *params)
{
    ...
    if (params->max_datagram_frame_size) {
        tp_data_len = snprintf(dgram_tp_str, 256, "max_datagram_frame_size=%" PRIu64 "\n",
                               params->max_datagram_frame_size);
        if (tp_data_len < 0) {
            return -XQC_ESYS;
        }
