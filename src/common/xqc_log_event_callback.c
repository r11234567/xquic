...
void
xqc_log_CON_SERVER_LISTENING_callback(xqc_log_t *log, const char *func,
                                      ...
                                      socklen_t peer_addrlen)
{
    ...
    if (peer_addr->sa_family == AF_INET) {
        xqc_qlog_implement(
            log, CON_SERVER_LISTENING, func, "|ip_v4:%s|port_v4:%d|",
            xqc_peer_addr_str(log->engine, (struct sockaddr *)sa_peer, peer_addrlen),
            ntohs(sa_peer->sin_port));
    } else {
        xqc_qlog_implement(
            log, CON_SERVER_LISTENING, func, "|ip_v6:%s|port_v6:%d|",
            xqc_peer_addr_str(log->engine, (struct sockaddr *)sa_peer, peer_addrlen),
            ntohs(sa_peer->sin_port));
    }
}
...
void
xqc_log_CON_CONNECTION_STARTED_callback(xqc_log_t *log, const char *func,
    ...
    if (local == XQC_LOG_LOCAL_EVENT) {
        ...
        xqc_qlog_implement(log, CON_CONNECTION_STARTED, func,
                           ...
                           xqc_peer_addr_str(conn->engine, (struct sockaddr *)sa_peer,
                                             conn->peer_addrlen),
                           ntohs(sa_peer->sin_port), log->scid,
                           xqc_dcid_str(conn->engine, &conn->dcid_set.current_dcid));
    }
}

void
xqc_log_CON_CONNECTION_CLOSED_callback(xqc_log_t *log, const char *func,
...
void
xqc_log_CON_CONNECTION_ID_UPDATED_callback(xqc_log_t *log, const char *func,
                                           xqc_connection_t *conn)
{
    unsigned char scid_str[XQC_MAX_CID_LEN * 2 + 1];
    xqc_hex_dump(scid_str, conn->scid_set.user_scid.cid_buf,
                 conn->scid_set.user_scid.cid_len);
    scid_str[conn->scid_set.user_scid.cid_len * 2] = '\0';
    xqc_qlog_implement(log, CON_CONNECTION_ID_UPDATED, func, "|scid:%s|dcid:%s|",
                       scid_str, conn->dcid_set.current_dcid_str);
}

void
xqc_log_CON_CONNECTION_STATE_UPDATED_callback(xqc_log_t *log, const char *func,
                                              xqc_connection_t *conn)
{
    xqc_qlog_implement(log, CON_CONNECTION_STATE_UPDATED, func, "|new:%s|",
                       xqc_conn_state_2_str(conn->conn_state));
}

void
xqc_log_CON_PATH_ASSIGNED_callback(xqc_log_t *log, const char *func, xqc_path_ctx_t *path,
                                   xqc_connection_t *conn)
{
    xqc_qlog_implement(log, CON_PATH_ASSIGNED, func,
                       ...
                       xqc_dcid_str(log->engine, &path->path_dcid),
                       xqc_scid_str(log->engine, &path->path_scid));
}

void
xqc_log_CON_MTU_UPDATED_callback(xqc_log_t *log, const char *func, xqc_connection_t *conn,
                                 int32_t is_done)
{
    xqc_qlog_implement(log, CON_MTU_UPDATED, func, "|new:%uz|done:%d|max:%uz|",
                       conn->pkt_out_size, is_done, conn->max_pkt_out_size);
}

void
xqc_log_SEC_KEY_UPDATED_callback(xqc_log_t *log, const char *func,
                                 xqc_engine_ssl_config_t ssl_config, xqc_int_t local)
{
    if (local == XQC_LOG_LOCAL_EVENT) {
        xqc_qlog_implement(log, SEC_KEY_UPDATED, func, "|local|ciphers:%s|",
                           ssl_config.ciphers);

    } else {
        xqc_qlog_implement(log, SEC_KEY_UPDATED, func, "|remote|ciphers:%s|",
                           ssl_config.ciphers);
    }
}

void
xqc_log_TRA_VERSION_INFORMATION_callback(xqc_log_t *log, const char *func,
                                         uint32_t local_count, uint32_t *local_version,
                                         uint32_t remote_count, uint32_t *remote_version,
                                         uint32_t choose)
{
    ...
    xqc_qlog_implement(log, TRA_VERSION_INFORMATION, func, "|%s|choose:%d|", log_buf,
                       choose);
}

void
xqc_log_TRA_ALPN_INFORMATION_callback(xqc_log_t *log, const char *func,
                                      const unsigned char *server_alpn_list,
                                      unsigned int server_alpn_list_len,
                                      const unsigned char *client_alpn_list,
                                      unsigned int client_alpn_list_len,
                                      const char *selected_alpn, size_t selected_alpn_len)
{
    ...
    xqc_qlog_implement(log, TRA_ALPN_INFORMATION, func, "|%s|selected_alpn:%*s|", log_buf,
                       selected_alpn_len, selected_alpn);
}

void
xqc_log_TRA_PARAMETERS_SET_callback(xqc_log_t *log, const char *func,
                                    xqc_connection_t *conn, xqc_int_t local)
{
    ...
    xqc_qlog_implement(log, TRA_PARAMETERS_SET, func,
                       ...
                       setting->disable_active_migration, setting->max_idle_timeout,
                       setting->max_udp_payload_size, setting->active_connection_id_limit,
                       setting->max_data);
}

void
xqc_log_TRA_PACKET_RECEIVED_callback(xqc_log_t *log, const char *func,
                                     xqc_packet_in_t *packet_in)
{
    xqc_qlog_implement(
        log, TRA_PACKET_RECEIVED, func,
        "|pkt_pns:%d|pkt_type:%s|pkt_num:%ui|len:%uz|frame_flag:%s|path_id:%ui|",
        packet_in->pi_pkt.pkt_pns, xqc_pkt_type_2_str(packet_in->pi_pkt.pkt_type),
        packet_in->pi_pkt.pkt_num, packet_in->buf_size,
        xqc_frame_type_2_str(log->engine, packet_in->pi_frame_types),
        packet_in->pi_path_id);
}

void
xqc_log_TRA_PACKET_DROPPED_callback(xqc_log_t *log, const char *func, const char *trigger,
                                    xqc_int_t ret, const char *pi_pkt_type,
                                    xqc_packet_number_t pi_pkt_num)
{
    xqc_qlog_implement(log, TRA_PACKET_DROPPED, func,
                       "|trigger:%s|ret:%d|pkt_type:%s|pkt_num:%ui|", trigger, ret,
                       pi_pkt_type, pi_pkt_num);
}
...
xqc_log_TRA_PACKET_SENT_callback(xqc_log_t *log, const char *func, xqc_connection_t *conn,
                                 ...
                                 xqc_usec_t send_time, ssize_t sent, xqc_bool_t with_pn)
{
    if (with_pn) {
        xqc_qlog_implement(
            log, TRA_PACKET_SENT, func,
            "|<==|conn:%p|path_id:%ui|pkt_pns:%d|pkt_type:%s|pkt_num:%ui|size:%d|frame_"
            "flag:%s|"
            ...
            xqc_frame_type_2_str(log->engine, packet_out->po_frame_types), sent,
            path->path_send_ctl->ctl_bytes_in_flight, send_time, packet_out->po_stream_id,
            packet_out->po_stream_offset);
    } else {
        xqc_qlog_implement(log, TRA_PACKET_SENT, func,
                           "|<==|conn:%p|path_id:%ui|pkt_type:%s|pkt_pns:%d|frame_flag:%"
                           "s|size:%ud|sent:%z|",
                           conn, path->path_id,
                           xqc_pkt_type_2_str(packet_out->po_pkt.pkt_type),
                           packet_out->po_pkt.pkt_pns,
                           xqc_frame_type_2_str(log->engine, packet_out->po_frame_types),
                           packet_out->po_used_size, sent);
    }
}

void
xqc_log_TRA_PACKET_BUFFERED_callback(xqc_log_t *log, const char *func,
                                     xqc_packet_in_t *packet_in)
{
    xqc_qlog_implement(log, TRA_PACKET_BUFFERED, func,
                       "|pkt_num:%ui|pkt_pns:%d|pkt_type:%d|len:%d|",
                       packet_in->pi_pkt.pkt_num, packet_in->pi_pkt.pkt_pns,
                       packet_in->pi_pkt.pkt_type, packet_in->buf_size);
}

void
xqc_log_TRA_PACKETS_ACKED_callback(xqc_log_t *log, const char *func,
                                   xqc_packet_in_t *packet_in, xqc_packet_number_t high,
                                   xqc_packet_number_t low, uint64_t path_id)
{
...
xqc_log_TRA_DATAGRAMS_SENT_callback(xqc_log_t *log, const char *func, ssize_t size,
                                    uint64_t path_id)
{
    xqc_qlog_implement(log, TRA_DATAGRAMS_SENT, func, "|size:%z|path_id:%ui|", size,
                       path_id);
}
...
xqc_log_TRA_DATAGRAMS_RECEIVED_callback(xqc_log_t *log, const char *func, ssize_t size,
                                        uint64_t path_id)
{
    xqc_qlog_implement(log, TRA_DATAGRAMS_RECEIVED, func, "|size:%d|path_id:%ui|", size,
                       path_id);
}

void
xqc_log_TRA_STREAM_STATE_UPDATED_callback(xqc_log_t *log, const char *func,
                                          xqc_stream_t *stream, xqc_int_t stream_type,
                                          xqc_int_t state)
{
    if (stream_type == XQC_LOG_STREAM_SEND) {
        xqc_qlog_implement(log, TRA_STREAM_STATE_UPDATED, func,
                           "|stream_id:%d|send_stream|old:%d|new:%d|", stream->stream_id,
                           stream->stream_state_send, state);
    } else {
        xqc_qlog_implement(log, TRA_STREAM_STATE_UPDATED, func,
                           "|stream_id:%d|recv_stream|old:%d|new:%d|", stream->stream_id,
...
xqc_log_TRA_FRAMES_PROCESSED_callback(xqc_log_t *log, const char *func, ...)
{
    ...
    case XQC_FRAME_PADDING: {
        ...
        xqc_qlog_implement(log, TRA_FRAMES_PROCESSED, func, "|type:%d|length:%ui|",
                           frame_type, length);
        break;
    }

    case XQC_FRAME_PING:
        xqc_qlog_implement(log, TRA_FRAMES_PROCESSED, func, "|type:%d|", frame_type);
        break;
    ...
    case XQC_FRAME_ACK: {
        ...
        for (int i = 0; i < ack_info->n_ranges; i++) {
            if (i == 0) {
                p = xqc_sprintf(p, last, "{%ui - %ui", ack_info->ranges[i].low,
                                ack_info->ranges[i].high);
        ...
        xqc_qlog_implement(log, TRA_FRAMES_PROCESSED, func,
                           "|type:%d|ack_delay:%ui|ack_range:%s|", frame_type,
                           ack_info->ack_delay, buf);
        break;
    }
    ...
    case XQC_FRAME_STOP_SENDING: {
        ...
        uint64_t err_code = va_arg(args, uint64_t);
        xqc_qlog_implement(log, TRA_FRAMES_PROCESSED, func,
                           "|type:%d|stream_id:%ui|err_code:%ui|", frame_type, stream_id,
                           err_code);
        break;
    }
    ...
    case XQC_FRAME_NEW_TOKEN: {
        ...
        xqc_qlog_implement(log, TRA_FRAMES_PROCESSED, func,
                           "|type:%d|token_length:%ui|token:%s|", frame_type, length,
                           token);
        break;
    }
    ...
    case XQC_FRAME_STREAM: {
        ...
        xqc_qlog_implement(log, TRA_FRAMES_PROCESSED, func,
                           "|type:%d|data_offset:%ui|data_length:%d|fin:%d|", frame_type,
                           frame->data_offset, frame->data_length, frame->fin);
        break;
    }

    case XQC_FRAME_DATAGRAM: {
        uint64_t length = va_arg(args, uint64_t);
        xqc_qlog_implement(log, TRA_FRAMES_PROCESSED, func, "|type:%d|data_length:%ui|",
                           frame_type, length);
        break;
    }
    ...
    case XQC_FRAME_MAX_DATA: {
        ...
        xqc_qlog_implement(log, TRA_FRAMES_PROCESSED, func, "|type:%d|max_data:%ui|",
                           frame_type, max_data);
        break;
    }
    ...
    case XQC_FRAME_MAX_STREAM_DATA: {
        ...
        xqc_qlog_implement(log, TRA_FRAMES_PROCESSED, func,
                           "|type:%d|stream_id:%ui|max_stream_data:%ui|", frame_type,
    ...
    case XQC_FRAME_DATA_BLOCKED: {
        ...
        xqc_qlog_implement(log, TRA_FRAMES_PROCESSED, func,
                           "|type:%d|bidirectional|limit:%ui|", frame_type, data_limit);
        break;
    }
    ...
    case XQC_FRAME_STREAM_DATA_BLOCKED: {
        ...
        xqc_qlog_implement(log, TRA_FRAMES_PROCESSED, func,
                           "|type:%d|bidirectional|stream_id:%ui|limit:%ui|", frame_type,
    ...
    case XQC_FRAME_NEW_CONNECTION_ID: {
        ...
        xqc_qlog_implement(log, TRA_FRAMES_PROCESSED, func,
                           "|type:%d|sequence_number:%ui|retire_prior_to:%ui|connection_"
                           "id_length:%d|connection_id:%s|",
                           frame_type, new_cid->cid_seq_num, retire_prior_to,
                           new_cid->cid_len, scid_str);
        break;
    }
    ...
    case XQC_FRAME_Extension: break;
...
void
xqc_log_TRA_STREAM_DATA_MOVED_callback(xqc_log_t *log, const char *func,
                                       xqc_stream_t *stream, xqc_bool_t is_recv,
                                       size_t read_or_write_size, size_t recv_buf_size,
                                       uint8_t fin, int ret, int pkt_type, int buff_1rtt,
                                       size_t offset)
{
    if (is_recv) {
        xqc_qlog_implement(log, TRA_STREAM_DATA_MOVED, func,
                           "|stream_id:%ui|read:%uz|recv_buf_size:%uz|fin:%d|stream_"
                           "length:%ui|next_read_offset:%ui|conn:%p"
        ...
        xqc_qlog_implement(
            log, TRA_STREAM_DATA_MOVED, func,
            ...
            xqc_pkt_type_2_str(pkt_type), buff_1rtt, read_or_write_size, offset, fin,
...
void
xqc_log_TRA_DATAGRAM_DATA_MOVED_callback(xqc_log_t *log, const char *func,
                                         xqc_stream_t *stream, size_t moved_data_len,
                                         const char *from, const char *to)
{
    xqc_qlog_implement(log, TRA_STREAM_DATA_MOVED, func,
                       "|stream_id:%ui|length:%uz|from:%s|to:%s|", stream->stream_id,
                       moved_data_len, from, to);
}

void
xqc_log_TRA_STATELESS_RESET_callback(xqc_log_t *log, const char *func,
                                     xqc_connection_t *conn)
{
...
void
xqc_log_REC_PARAMETERS_SET_callback(xqc_log_t *log, const char *func,
                                    xqc_send_ctl_t *send_ctl, uint8_t timer_granularity,
                                    xqc_cc_params_t cc_params)
{
    xqc_qlog_implement(log, REC_PARAMETERS_SET, func,
                       "|reordering_threshold:%ui|time_threshold:%d|timer_granularity:%d|"
                       "initial_rtt:%ui|"
                       "initial_congestion_window:%d|minimum_congestion_window:%d|",
                       send_ctl->ctl_reordering_packet_threshold,
                       send_ctl->ctl_reordering_time_threshold_shift, timer_granularity,
                       send_ctl->ctl_srtt / 1000, cc_params.init_cwnd,
                       cc_params.min_cwnd);
}

void
xqc_log_REC_METRICS_UPDATED_callback(xqc_log_t *log, const char *func,
                                     xqc_send_ctl_t *send_ctl)
{
    ...
    if (send_ctl->ctl_cong_callback->xqc_cong_ctl_init_bbr) {
        bw = send_ctl->ctl_cong_callback->xqc_cong_ctl_get_bandwidth_estimate(
            send_ctl->ctl_cong);
        pacing_rate =
            send_ctl->ctl_cong_callback->xqc_cong_ctl_get_pacing_rate(send_ctl->ctl_cong);
        mode =
            send_ctl->ctl_cong_callback->xqc_cong_ctl_info_cb->mode(send_ctl->ctl_cong);
        min_rtt = send_ctl->ctl_cong_callback->xqc_cong_ctl_info_cb->min_rtt(
            send_ctl->ctl_cong);
        xqc_qlog_implement(log, REC_METRICS_UPDATED, func,
                           "|cwnd:%ui|inflight:%ud|mode:%ud|applimit:%ud|pacing_rate:%ui|"
                           "bw:%ui|srtt:%ui|"
                           "latest_rtt:%ui|ctl_rttvar:%ui|pto_count:%ud|min_rtt:%ui|send:"
                           "%ud|lost:%ud|tlp:%ud|recv:%ud|",
                           cwnd, send_ctl->ctl_bytes_in_flight, mode,
                           send_ctl->ctl_app_limited, pacing_rate, bw, send_ctl->ctl_srtt,
                           send_ctl->ctl_latest_rtt, send_ctl->ctl_pto_count, min_rtt,
                           send_ctl->ctl_send_count, send_ctl->ctl_lost_count,
                           send_ctl->ctl_tlp_count, send_ctl->ctl_recv_count);

    } else {
        xqc_qlog_implement(
            log, REC_METRICS_UPDATED, func,
            "|cwnd:%ui|inflight:%ud|applimit:%ud|srtt:%ui|latest_rtt:%ui|pto_count:%ud|"
            "send:%ud|lost:%ud|tlp:%ud|recv:%ud|",
            cwnd, send_ctl->ctl_bytes_in_flight, send_ctl->ctl_app_limited,
            send_ctl->ctl_srtt, send_ctl->ctl_latest_rtt, send_ctl->ctl_pto_count,
            send_ctl->ctl_send_count, send_ctl->ctl_lost_count, send_ctl->ctl_tlp_count,
            send_ctl->ctl_recv_count);
    }
}

void
xqc_log_REC_CONGESTION_STATE_UPDATED_callback(xqc_log_t *log, const char *func,
                                              char *new_state)
{
    xqc_qlog_implement(log, REC_CONGESTION_STATE_UPDATED, func, "|new_state:%s|",
                       new_state);
}
...
xqc_log_REC_LOSS_TIMER_UPDATED_callback(xqc_log_t *log, const char *func,
                                        ...
                                        xqc_usec_t inter_time, xqc_int_t type,
                                        xqc_int_t event)
{
    ...
    if (event == XQC_LOG_TIMER_SET) {
        xqc_qlog_implement(log, REC_LOSS_TIMER_UPDATED, func,
                           ...
                           timer_manager->timer[type].expire_time, inter_time);

    } else if (event == XQC_LOG_TIMER_EXPIRE) {
        xqc_qlog_implement(log, REC_LOSS_TIMER_UPDATED, func,
                           "|event_type:expired|type:%s|expire_time:%ui|",
                           xqc_timer_type_2_str(type),
                           timer_manager->timer[type].expire_time);
...
void
xqc_log_REC_PACKET_LOST_callback(xqc_log_t *log, const char *func,
                                 xqc_packet_out_t *packet_out,
                                 xqc_packet_number_t lost_pn, xqc_usec_t lost_send_time,
                                 xqc_usec_t loss_delay)
{
    xqc_qlog_implement(
        log, REC_PACKET_LOST, func,
        "|pkt_pns:%d|pkt_type:%d|pkt_num:%d|lost_pn:%ui|po_sent_time:%ui|"
        "lost_send_time:%ui|loss_delay:%ui|frame:%s|repair:%d|path_id:%ui|",
        packet_out->po_pkt.pkt_pns, packet_out->po_pkt.pkt_type,
        packet_out->po_pkt.pkt_num, lost_pn, packet_out->po_sent_time, lost_send_time,
        loss_delay, xqc_frame_type_2_str(log->engine, packet_out->po_frame_types),
        XQC_NEED_REPAIR(packet_out->po_frame_types), packet_out->po_path_id);
}

void
xqc_log_HTTP_PARAMETERS_SET_callback(xqc_log_t *log, const char *func,
                                     xqc_h3_conn_t *h3_conn, xqc_int_t local)
{
    ...
    xqc_qlog_implement(log, HTTP_PARAMETERS_SET, func,
                       "|owner:%s|max_field_section_size:%ui|qpack_max_table_capacity:%"
                       "ui|qpack_blocked_streams:%ui|",
                       local == XQC_LOG_LOCAL_EVENT ? "local" : "remote",
                       setting->max_field_section_size,
                       setting->qpack_dec_max_table_capacity,
                       setting->qpack_blocked_streams);
}

void
xqc_log_HTTP_PARAMETERS_RESTORED_callback(xqc_log_t *log, const char *func,
                                          xqc_h3_conn_t *h3_conn)
{
    xqc_h3_conn_settings_t *setting = &h3_conn->local_h3_conn_settings;
    xqc_qlog_implement(log, HTTP_PARAMETERS_RESTORED, func,
                       "|max_field_section_size:%ui|qpack_max_table_capacity:%ui|qpack_"
                       "blocked_streams:%ui|",
                       setting->max_field_section_size,
                       setting->qpack_dec_max_table_capacity,
                       setting->qpack_blocked_streams);
...
xqc_log_HTTP_FRAME_CREATED_callback(xqc_log_t *log, const char *func, ...)
{
    ...
    case XQC_H3_FRM_HEADERS: {
        ...
        for (uint32_t i = 0; i < headers->count; ++i) {
            ...
            if (header->value.iov_len > 0) {
                p = xqc_sprintf(p, last, "{name:%*s} {value:%*s}|",
                                (size_t)header->name.iov_len, header->name.iov_base,
                                (size_t)header->value.iov_len, header->value.iov_base);
            } else {
                p = xqc_sprintf(p, last, "{name:%*s}|", (size_t)header->name.iov_len,
                                header->name.iov_base);
            }
        }
        xqc_qlog_implement(log, HTTP_FRAME_CREATED, func, "|stream_id:%ui|type:%d|%s",
                           stream_id, type, log_buf);
        break;
    }
    ...
    case XQC_H3_FRM_MAX_PUSH_ID: {
        ...
        xqc_qlog_implement(log, HTTP_FRAME_CREATED, func,
                           "|stream_id:%ui|type:%d|push_id:%ui|", stream_id, type,
                           push_id);
        break;
    }
...
void
xqc_log_HTTP_FRAME_PARSED_callback(xqc_log_t *log, const char *func,
                                   xqc_h3_stream_t *h3_stream)
{
    ...
    case XQC_H3_FRM_SETTINGS:
        xqc_qlog_implement(log, HTTP_FRAME_PARSED, func,
                           "|stream_id:%ui|type:%d|length:%ui|", stream_id, frame->type,
                           frame->len);
        break;
    case XQC_H3_FRM_CANCEL_PUSH: {
        xqc_qlog_implement(log, HTTP_FRAME_PARSED, func,
                           "|stream_id:%ui|type:%d|length:%ui|push_id:%ui|", stream_id,
                           frame->type, frame->len,
                           frame->frame_payload.cancel_push.push_id.vi);
        break;
    }
    case XQC_H3_FRM_PUSH_PROMISE: {
        xqc_qlog_implement(log, HTTP_FRAME_PARSED, func,
                           "|stream_id:%ui|type:%d|length:%ui|push_id:%ui|", stream_id,
                           frame->type, frame->len,
                           frame->frame_payload.push_promise.push_id.vi);
        break;
    }
    case XQC_H3_FRM_GOAWAY: {
        xqc_qlog_implement(log, HTTP_FRAME_PARSED, func,
                           "|stream_id:%ui|type:%d|length:%ui|push_id:%ui|", stream_id,
                           frame->type, frame->len,
                           frame->frame_payload.goaway.stream_id.vi);
        break;
    }
    case XQC_H3_FRM_MAX_PUSH_ID: {
        xqc_qlog_implement(log, HTTP_FRAME_PARSED, func,
                           "|stream_id:%ui|type:%d|length:%ui|push_id:%ui|", stream_id,
                           frame->type, frame->len,
                           frame->frame_payload.max_push_id.push_id.vi);
        break;
    }
    default: break;
    }
...
void
xqc_log_HTTP_PRIORITY_UPDATED_callback(xqc_log_t *log, const char *func,
                                       xqc_h3_priority_t *prio, xqc_h3_stream_t *h3s)
{
    xqc_qlog_implement(
        log, HTTP_PRIORITY_UPDATED, func,
        "|urgency:%ui|incremental:%ui|schedule:%ui|reinject:%ui|fec_bm_mode:%ui|"
        "stream_id:%ui|conn:%p|",
        prio->urgency, prio->incremental, prio->schedule, prio->reinject,
        h3s->stream->stream_fec_blk_mode, h3s->stream_id, h3s->h3c->conn);
}

void
xqc_log_QPACK_STREAM_STATE_UPDATED_callback(xqc_log_t *log, const char *func,
                                            xqc_h3_stream_t *h3_stream)
{
    xqc_qlog_implement(
        log, QPACK_STREAM_STATE_UPDATED, func, "|stream_id:%ui|%s|", h3_stream->stream_id,
        h3_stream->flags & XQC_HTTP3_STREAM_FLAG_QPACK_DECODE_BLOCKED ? "blocked"
                                                                      : "unblocked");
}
...
xqc_log_QPACK_DYNAMIC_TABLE_UPDATED_callback(xqc_log_t *log, const char *func, ...)
{
    ...
    if (type == XQC_LOG_DTABLE_EVICTED) {
        xqc_qlog_implement(log, QPACK_DYNAMIC_TABLE_UPDATED, func, "|evicted|index:%ui|",
                           index);
        ...
        xqc_qlog_implement(log, QPACK_DYNAMIC_TABLE_UPDATED, func,
                           "|inserted|index:%ui|name:%*s|value:%*s|", index, (size_t)nlen,
                           name, (size_t)vlen, value);
    }
    va_end(args);
}
    ...
    if (coder_type == XQC_LOG_ENCODER_EVENT) {
        ...
        case XQC_INS_TYPE_ENC_SET_DTABLE_CAP:
            xqc_qlog_implement(log, QPACK_INSTRUCTION_PARSED, func,
                               "|set_dynamic_table_capacity|capacity:%ui|",
                               ctx->capacity.value);
            break;
        case XQC_INS_TYPE_ENC_INSERT_NAME_REF:
            xqc_qlog_implement(log, QPACK_INSTRUCTION_PARSED, func,
                               "|insert_with_name_reference|name_index:%ui|value:%*s|",
                               ctx->name_index.value, (size_t)ctx->value->value->data_len,
        ...
        case XQC_INS_TYPE_ENC_INSERT_LITERAL:
            xqc_qlog_implement(log, QPACK_INSTRUCTION_PARSED, func,
                               ...
                               (size_t)ctx->value->value->data_len,
                               ctx->value->value->data);
            break;
        case XQC_INS_TYPE_ENC_DUP:
            xqc_qlog_implement(log, QPACK_INSTRUCTION_PARSED, func,
        ...
        case XQC_INS_TYPE_DEC_SECTION_ACK:
            xqc_qlog_implement(log, QPACK_INSTRUCTION_PARSED, func,
                               "|header_acknowledgement|stream_id:%ui|",
                               ctx->stream_id.value);
            break;
        case XQC_INS_TYPE_DEC_STREAM_CANCEL:
            xqc_qlog_implement(log, QPACK_INSTRUCTION_PARSED, func,
                               "|stream_cancellation|stream_id:%ui|",
                               ctx->stream_id.value);
            break;
        case XQC_INS_TYPE_DEC_INSERT_CNT_INC:
            xqc_qlog_implement(log, QPACK_INSTRUCTION_PARSED, func,
                               "|insert_count_increment|increment:%ui|",
                               ctx->increment.value);
            break;
        }
    }
