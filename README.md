> **Fork Notice**
>
> This repository is a fork of [alibaba/xquic](https://github.com/alibaba/xquic), maintained primarily
...
## Synchronized fork fixes

- Certificate verification now supports `XQC_TLS_CERT_FLAG_APP_VERIFY`. It
  passes the complete peer-presented chain, leaf first, to the application's
  `cert_verify_cb` and treats that decision as final. The mode implies peer
  verification and fails closed when no callback is installed. The normal
  `XQC_TLS_CERT_FLAG_NEED_VERIFY` path remains library-owned and uses the TLS
  backend's default trust paths.
- ~~The fork raised the fixed stream-reassembly node cap and made cap pressure
  a non-fatal, unacknowledged packet drop.~~ **Replaced by the stronger
  `mqvpn-main` implementation:** the cap is configurable per connection, one
  slot is reserved for a frame that can fill the leftmost gap, repeated cap
