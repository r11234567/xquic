...
ssize_t xqc_h3_stream_write_data_to_buffer(xqc_h3_stream_t *h3s,
                                           unsigned char *data,
                                           uint64_t data_size, uint8_t fin);
xqc_int_t xqc_decoder_copy_header(xqc_http_header_t *hdr, xqc_var_buf_t *name,
                                  xqc_var_buf_t *value);
/* not exposed in xqc_h3_stream.h, but stable file-scope entry used by tests */
xqc_int_t xqc_h3_stream_process_in(xqc_h3_stream_t *h3s, unsigned char *data,
                                   size_t data_len, xqc_bool_t fin_flag);

ssize_t xqc_test_frame_parse(const char *p, size_t sz,
                             xqc_h3_frame_pctx_t *state) {
  ssize_t offset = 0;
  while (offset < sz) {
...
void xqc_test_frame() {
  ...
  ret =
      xqc_h3_frm_write_push_promise(&send_buf, push_id, push_promise, XQC_TRUE);
  CU_ASSERT(ret == XQC_OK);
  /* write goaway frame */
  ...
  processed = xqc_test_frame_parse(buf->data + buf->consumed_len,
                                   buf->data_len - buf->consumed_len, &pctx);
  ...
  CU_ASSERT(pctx.frame.frame_payload.push_promise.encoded_field_section !=
            NULL);
  if (pctx.frame.frame_payload.push_promise.encoded_field_section != NULL) {
    for (int i = 0; i < strlen(data); i++) {
      CU_ASSERT(
          data[i] ==
          pctx.frame.frame_payload.push_promise.encoded_field_section->data[i]);
    }
  }
  buf->consumed_len += processed;
  xqc_h3_frm_reset_pctx(&pctx);
  ...
  char reserved_frame[] = "\xcf\x25\x7c\x52\x89\x59\xd7\xba\x0a\xff\xff\xff\xff"
                          "\xff\xff\xff\xff\xff\xff";
  size_t reserved_frame_len = sizeof(reserved_frame) - 1;
  size_t reserved_consumed_len = 0;
  for (size_t i = 0; i < reserved_frame_len; i++) {
    processed =
        xqc_h3_frm_parse(reserved_frame + reserved_consumed_len, 1, &pctx);
    CU_ASSERT(processed > 0);
...
void xqc_test_h3_single_vint_frame_length_error() {
  ...
  for (size_t i = 0; i < sizeof(overlong) / sizeof(overlong[0]); i++) {
    ...
    processed =
        xqc_h3_frm_parse(overlong[i] + 3, sizeof(overlong[i]) - 3, &pctx);
  ...
  for (size_t i = 0; i < sizeof(short_payload) / sizeof(short_payload[0]);
       i++) {
    xqc_h3_frame_pctx_t pctx;
    memset(&pctx, 0, sizeof(pctx));

    ssize_t processed =
        xqc_h3_frm_parse(short_payload[i], sizeof(short_payload[i]), &pctx);
    CU_ASSERT(processed == -XQC_H3_DECODE_ERROR);
    CU_ASSERT(pctx.state == XQC_H3_FRM_STATE_PAYLOAD);
    CU_ASSERT(pctx.frame.consumed_len == 2);
...
void xqc_test_ins() {
  ...
  ret = xqc_ins_write_insert_literal_name(buf, name, strlen(name), value,
                                          strlen(value));
  CU_ASSERT(ret == XQC_OK);
  /* write Duplicate */
  ...
  processed = xqc_ins_parse_encoder(buf->data + buf->consumed_len,
                                    buf->data_len - buf->consumed_len, enc_ctx);
  CU_ASSERT(processed > 0);
  ...
  processed = xqc_ins_parse_decoder(buf->data + buf->consumed_len,
                                    buf->data_len - buf->consumed_len, dec_ctx);
  CU_ASSERT(processed > 0);
...
void xqc_test_rep() {
  ...
  CU_ASSERT(ret == XQC_OK);
  ret =
      xqc_rep_write_indexed(buf, XQC_DTABLE_FLAG, xqc_abs2brel(base, index_2));
  CU_ASSERT(ret == XQC_OK);
  ret = xqc_rep_write_indexed(buf, XQC_STABLE_FLAG, index_3);
  CU_ASSERT(ret == XQC_OK);
  ret = xqc_rep_write_literal_name_value(buf, 1, strlen(name), name,
                                         strlen(value), value);
  CU_ASSERT(ret == XQC_OK);
  ret = xqc_rep_write_literal_with_pb_name_ref(
      buf, 1, xqc_abs2pbrel(base, index_1), strlen(value), value);
  CU_ASSERT(ret == XQC_OK);
  ret = xqc_rep_write_literal_with_name_ref(buf, 1, XQC_DTABLE_FLAG,
                                            xqc_abs2brel(base, index_2),
                                            strlen(value), value);
  CU_ASSERT(ret == XQC_OK);
  ret = xqc_rep_write_literal_with_name_ref(buf, 1, XQC_STABLE_FLAG, index_3,
                                            strlen(value), value);
  CU_ASSERT(ret == XQC_OK);

  ssize_t processed = xqc_rep_decode_prefix(ctx, max_entries, insert_count,
                                            buf->data + buf->consumed_len,
  ...
  processed = xqc_rep_decode_field_line(ctx, buf->data + buf->consumed_len,
                                        buf->data_len - buf->consumed_len);
  CU_ASSERT(processed > 0);
  ...
  CU_ASSERT(ret == XQC_OK);
  processed = xqc_rep_decode_prefix(ctx, max_entries, insert_count,
                                    buf->data + buf->consumed_len,
                                    buf->data_len - buf->consumed_len);
  CU_ASSERT(processed > 0);
  CU_ASSERT(ctx->sign == 0);
...
void xqc_test_stream() {
  ...
  xqc_stream_t *stream = xqc_create_stream_with_conn(
      conn, XQC_UNDEFINE_STREAM_ID, XQC_CLI_UNI, NULL, NULL);
  ...
  xqc_h3_stream_t *h3s =
      xqc_h3_stream_create(h3c, stream, XQC_H3_STREAM_TYPE_CONTROL, NULL);
  CU_ASSERT(h3s != NULL);
  ...
  ssize_t n_write =
      xqc_h3_stream_write_data_to_buffer(h3s, data, data_size, XQC_TRUE);
...
static void xqc_h3_critical_run_case(xqc_connection_t *conn, xqc_h3_conn_t *h3c,
  ...
  xqc_h3_stream_t *h3s = xqc_h3_stream_create(
      h3c, stream, (xqc_h3_stream_type_t)tc->stream_type, NULL);
  ...
  if (tc->expected_conn_err == H3_CLOSED_CRITICAL_STREAM) {
    CU_ASSERT(XQC_CONN_ERR_CODE(conn->conn_err) == H3_CLOSED_CRITICAL_STREAM);
  } else {
    CU_ASSERT(XQC_CONN_ERR_CODE(conn->conn_err) == tc->expected_conn_err);
  }
...
void xqc_test_h3_critical_stream_close() {
  ...
  xqc_h3_critical_close_case_t cases[] = {
      ...
      {"control_running", XQC_H3_STREAM_TYPE_CONTROL, XQC_FALSE, XQC_FALSE, 0,
       H3_CLOSED_CRITICAL_STREAM, XQC_TRUE},
      /* qpack encoder stream peer-close on a healthy connection */
      {"qpack_encoder_running", XQC_H3_STREAM_TYPE_QPACK_ENCODER, XQC_FALSE,
       XQC_FALSE, 0, H3_CLOSED_CRITICAL_STREAM, XQC_TRUE},
      /* qpack decoder stream peer-close on a healthy connection */
      {"qpack_decoder_running", XQC_H3_STREAM_TYPE_QPACK_DECODER, XQC_FALSE,
       XQC_FALSE, 0, H3_CLOSED_CRITICAL_STREAM, XQC_TRUE},
      /* request stream close must not raise the critical-stream error */
      {"request_running", XQC_H3_STREAM_TYPE_REQUEST, XQC_FALSE, XQC_FALSE, 0,
       0, XQC_FALSE},
      /* push stream close must not raise the critical-stream error */
      {"push_running", XQC_H3_STREAM_TYPE_PUSH, XQC_FALSE, XQC_FALSE, 0, 0,
       XQC_FALSE},
      /* unknown stream type must not raise the critical-stream error */
      {"unknown_running", XQC_H3_STREAM_TYPE_UNKNOWN, XQC_FALSE, XQC_FALSE, 0,
       0, XQC_FALSE},
      /* connection already issued CONNECTION_CLOSE - report is suppressed */
      {"control_closing_notify", XQC_H3_STREAM_TYPE_CONTROL, XQC_TRUE,
       XQC_FALSE, 0, 0, XQC_FALSE},
      /* connection state already CLOSING - report is suppressed */
      {"control_state_closing", XQC_H3_STREAM_TYPE_CONTROL, XQC_FALSE, XQC_TRUE,
       0, 0, XQC_FALSE},
      /* a previous conn_err must win (XQC_H3_CONN_ERR is first-write-wins) */
      {"control_preserve_err", XQC_H3_STREAM_TYPE_CONTROL, XQC_FALSE, XQC_FALSE,
       0xdead, 0xdead, XQC_FALSE},
  };
...
static void xqc_test_h3_second_stream_one(uint64_t stype,
                                          uint64_t expected_err_second) {
  xqc_connection_t *conn = test_engine_connect();
  CU_ASSERT_FATAL(conn != NULL);
...
static void xqc_test_h3_push_stream_rejected(xqc_conn_type_t conn_type,
  ...
  xqc_int_t ret =
      xqc_h3_conn_on_uni_stream_created(h3c, XQC_H3_STREAM_TYPE_PUSH);
  CU_ASSERT(ret == -XQC_H3_INVALID_STREAM);
 ...
 */
static const unsigned char xqc_h3_msgerr_valid_headers[] = {0x01, 0x03, 0x00,
                                                            0x00, 0xC0};

static xqc_h3_stream_t *xqc_h3_msgerr_setup(xqc_connection_t **out_conn,
                                            xqc_h3_conn_t **out_h3c) {
  xqc_connection_t *conn = test_engine_connect();
  if (conn == NULL) {
    return NULL;
  }
  ...
  xqc_h3_stream_t *h3s =
      xqc_h3_stream_create(h3c, stream, XQC_H3_STREAM_TYPE_REQUEST, NULL);
  if (h3s == NULL) {
    return NULL;
  }
...
static void xqc_h3_msgerr_teardown(xqc_h3_stream_t *h3s, xqc_h3_conn_t *h3c,
...
void xqc_test_h3_message_error_enum() { CU_ASSERT(H3_MESSAGE_ERROR == 0x10E); }
...
void xqc_test_h3_forbidden_headers_rejected() {
  ...
  CU_ASSERT(xqc_h3_hdr_is_forbidden((const unsigned char *)"transfer-encoding",
                                    17, (const unsigned char *)"chunked",
                                    7) == XQC_TRUE);
  ...
  CU_ASSERT(xqc_h3_hdr_is_forbidden((const unsigned char *)"proxy-connection",
                                    16, (const unsigned char *)"keep-alive",
                                    10) == XQC_TRUE);
...
void xqc_test_h3_malformed_headers_uses_message_error() {
  ...
  xqc_int_t ret = xqc_h3_stream_process_in(h3s, buf, sizeof(buf), XQC_TRUE);
  ...
  CU_ASSERT(h3s->stream->stream_state_send == XQC_SEND_STREAM_ST_RESET_SENT);
  CU_ASSERT(conn->conn_err == 0);
  CU_ASSERT((conn->conn_flag & XQC_CONN_FLAG_ERROR) == 0);
...
static xqc_h3_stream_t *xqc_h3_ctrl_test_setup(xqc_connection_t **out_conn,
                                               xqc_h3_conn_t **out_h3c) {
  xqc_connection_t *conn = test_engine_connect();
  if (conn == NULL) {
    return NULL;
...
static void xqc_h3_ctrl_test_teardown(xqc_h3_stream_t *h3s, xqc_h3_conn_t *h3c,
                                      xqc_connection_t *conn) {
  xqc_stream_t *stream = h3s->stream;
  stream->stream_flag |= XQC_STREAM_FLAG_DISCARDED;
  xqc_h3_stream_destroy(h3s);
 ...
 * SETTINGS: type=0x04, len=0x02, entry=[id=0x06(MAX_FIELD_SECTION_SIZE),
 * val=0x00]
 */
static ssize_t xqc_h3_ctrl_feed_settings(xqc_h3_stream_t *h3s) {
  unsigned char settings[] = {0x04, 0x02, 0x06, 0x00};
  return xqc_h3_stream_process_control(h3s, settings, sizeof(settings));
}

static ssize_t xqc_h3_ctrl_feed_setting(xqc_h3_stream_t *h3s,
                                        unsigned char identifier,
                                        unsigned char value) {
  unsigned char settings[] = {XQC_H3_FRM_SETTINGS, 0x02, identifier, value};
  return xqc_h3_stream_process_control(h3s, settings, sizeof(settings));
}

void xqc_test_h3_settings_accepted() {
  const xqc_conn_type_t roles[] = {XQC_CONN_TYPE_CLIENT, XQC_CONN_TYPE_SERVER};
  ...
  for (size_t i = 0; i < sizeof(roles) / sizeof(roles[0]); i++) {
    for (size_t j = 0; j < sizeof(identifiers) / sizeof(identifiers[0]); j++) {
      xqc_connection_t *conn = NULL;
      xqc_h3_conn_t *h3c = NULL;
      ...
      ssize_t processed = xqc_h3_ctrl_feed_setting(h3s, identifiers[j], 1);

      CU_ASSERT(processed == 4);
      CU_ASSERT(conn->conn_err == 0);
      CU_ASSERT(h3c->flags & XQC_H3_CONN_FLAG_SETTINGS_RECVED);
      if (identifiers[j] == XQC_H3_SETTINGS_MAX_FIELD_SECTION_SIZE) {
        CU_ASSERT(h3c->peer_h3_conn_settings.max_field_section_size == 1);
      }
...
void xqc_test_h3_reserved_h2_settings_rejected() {
  ...
  for (size_t i = 0; i < sizeof(roles) / sizeof(roles[0]); i++) {
    for (size_t j = 0; j < sizeof(identifiers) / sizeof(identifiers[0]); j++) {
      ...
      conn->conn_type = roles[i];
      ssize_t processed = xqc_h3_ctrl_feed_setting(h3s, identifiers[j], 0);

      CU_ASSERT(processed == -H3_SETTINGS_ERROR);
      CU_ASSERT(XQC_CONN_ERR_CODE(conn->conn_err) == H3_SETTINGS_ERROR);
...
static ssize_t xqc_h3_ctrl_feed_max_push_id(xqc_h3_stream_t *h3s,
...
static ssize_t xqc_h3_ctrl_feed_cancel_push(xqc_h3_stream_t *h3s,
                                            unsigned char push_id) {
  unsigned char frame[] = {XQC_H3_FRM_CANCEL_PUSH, 0x01, push_id};
  return xqc_h3_stream_process_control(h3s, frame, sizeof(frame));
}

static ssize_t xqc_h3_ctrl_feed_goaway(xqc_h3_stream_t *h3s,
                                       unsigned char identifier) {
...
void xqc_test_h3_goaway_id_increase_rejected() {
  ...
  for (size_t i = 0; i < sizeof(roles) / sizeof(roles[0]); i++) {
    ...
    CU_ASSERT(xqc_h3_ctrl_feed_goaway(h3s, 8) == -XQC_H3_INVALID_GOAWAY_ID);
...
void xqc_test_h3_reserved_control_frame_accepted() {
  ...
  CU_ASSERT(xqc_h3_stream_process_control(h3s, reserved, sizeof(reserved)) ==
            sizeof(reserved));
  CU_ASSERT(conn->conn_err == 0);
  CU_ASSERT((conn->conn_flag & XQC_CONN_FLAG_ERROR) == 0);
...
void xqc_test_h3_h2_reserved_frames_rejected() {
  ...
  for (size_t i = 0; i < sizeof(frame_types); ++i) {
    ...
    CU_ASSERT(xqc_h3_stream_process_in(h3s, frame, sizeof(frame), XQC_FALSE) ==
...
static void xqc_test_h3_ctrl_reject_data(void) {
  xqc_connection_t *conn = NULL;
  xqc_h3_conn_t *h3c = NULL;
  xqc_h3_stream_t *h3s = xqc_h3_ctrl_test_setup(&conn, &h3c);
  CU_ASSERT_FATAL(h3s != NULL);
  ...
  ssize_t processed =
      xqc_h3_stream_process_control(h3s, data_frame, sizeof(data_frame));
...
static void xqc_test_h3_ctrl_reject_zero_len_data(void) {
  xqc_connection_t *conn = NULL;
  xqc_h3_conn_t *h3c = NULL;
  xqc_h3_stream_t *h3s = xqc_h3_ctrl_test_setup(&conn, &h3c);
  CU_ASSERT_FATAL(h3s != NULL);
  ...
  ssize_t processed =
      xqc_h3_stream_process_control(h3s, zero_data, sizeof(zero_data));
...
static void xqc_test_h3_ctrl_reject_headers(void) {
  xqc_connection_t *conn = NULL;
  xqc_h3_conn_t *h3c = NULL;
  xqc_h3_stream_t *h3s = xqc_h3_ctrl_test_setup(&conn, &h3c);
  CU_ASSERT_FATAL(h3s != NULL);
  ...
  ssize_t processed =
      xqc_h3_stream_process_control(h3s, headers_frame, sizeof(headers_frame));
...
static void xqc_test_h3_ctrl_reject_push_promise(void) {
  xqc_connection_t *conn = NULL;
  xqc_h3_conn_t *h3c = NULL;
  xqc_h3_stream_t *h3s = xqc_h3_ctrl_test_setup(&conn, &h3c);
  CU_ASSERT_FATAL(h3s != NULL);
  ...
  ssize_t processed =
      xqc_h3_stream_process_control(h3s, push_promise, sizeof(push_promise));
 ...
 */
static void xqc_test_h3_ctrl_reject_data_before_settings(void) {
  xqc_connection_t *conn = NULL;
  xqc_h3_conn_t *h3c = NULL;
  xqc_h3_stream_t *h3s = xqc_h3_ctrl_test_setup(&conn, &h3c);
  CU_ASSERT_FATAL(h3s != NULL);
 ...
 */
static void xqc_test_h3_ctrl_goaway_then_data(void) {
  xqc_connection_t *conn = NULL;
  xqc_h3_conn_t *h3c = NULL;
  xqc_h3_stream_t *h3s = xqc_h3_ctrl_test_setup(&conn, &h3c);
  CU_ASSERT_FATAL(h3s != NULL);
 ...
 */
static void xqc_test_h3_ctrl_reject_partial_feed(void) {
  xqc_connection_t *conn = NULL;
  xqc_h3_conn_t *h3c = NULL;
  xqc_h3_stream_t *h3s = xqc_h3_ctrl_test_setup(&conn, &h3c);
  CU_ASSERT_FATAL(h3s != NULL);
  ...
  ssize_t processed =
      xqc_h3_stream_process_control(h3s, type_byte, sizeof(type_byte));
...
static void xqc_test_h3_ctrl_goaway_regression(void) {
  xqc_connection_t *conn = NULL;
  xqc_h3_conn_t *h3c = NULL;
  xqc_h3_stream_t *h3s = xqc_h3_ctrl_test_setup(&conn, &h3c);
  CU_ASSERT_FATAL(h3s != NULL);
  ...
  ssize_t processed =
      xqc_h3_stream_process_control(h3s, goaway, sizeof(goaway));
...
void xqc_test_h3_control_frame_unexpected(void) {
 ...
 */
static void xqc_test_h3_missing_settings_one(uint64_t frame_type) {
  xqc_connection_t *conn = test_engine_connect();
  CU_ASSERT_FATAL(conn != NULL);
  ...
  ssize_t processed =
      xqc_h3_stream_process_control(h3s, frame_buf, sizeof(frame_buf));
...
static void xqc_test_h3_request_frame_unexpected_one(uint64_t frame_type,
  ...
  ssize_t processed = xqc_h3_stream_process_request(
      h3s, frame_buf, sizeof(frame_buf), XQC_FALSE);
...
void xqc_test_h3_data_after_headers_accepted() {
  ...
  ssize_t processed =
      xqc_h3_stream_process_request(h3s, headers, sizeof(headers), XQC_FALSE);
  CU_ASSERT(processed == (ssize_t)sizeof(headers));
  CU_ASSERT(h3s->h3r->completed_header_count == 1);

  processed = xqc_h3_stream_process_request(h3s, data, sizeof(data), XQC_FALSE);
...
void xqc_test_h3_data_before_headers_rejected() {
  ...
  unsigned char data[] = {XQC_H3_FRM_DATA, 0x01, 0x2a};
  ssize_t processed =
      xqc_h3_stream_process_request(h3s, data, sizeof(data), XQC_FALSE);
  ...
  processed =
      xqc_h3_stream_process_in(h3s, empty_data, sizeof(empty_data), XQC_FALSE);
...
void xqc_test_h3_server_reserved_request_frame_accepted() {
  ...
  ssize_t processed = xqc_h3_stream_process_request(
      h3s, reserved_frame, sizeof(reserved_frame), XQC_FALSE);
...
void xqc_test_h3_server_push_promise_rejected() {
  ...
  unsigned char push_promise[] = {0x05, 0x02, 0x00, 0x00};
  ssize_t processed = xqc_h3_stream_process_request(
      h3s, push_promise, sizeof(push_promise), XQC_FALSE);
  ...
  processed = xqc_h3_stream_process_in(h3s, push_promise, sizeof(push_promise),
                                       XQC_FALSE);
...
void xqc_test_h3_allowed_headers_pass() {
  ...
  CU_ASSERT(xqc_h3_hdr_is_forbidden((const unsigned char *)"te", 2,
                                    (const unsigned char *)"trailers",
                                    8) == XQC_FALSE);

  /* te with value "Trailers" (mixed case): QPACK sends lowercase, reject
   * non-exact */
  CU_ASSERT(xqc_h3_hdr_is_forbidden((const unsigned char *)"te", 2,
                                    (const unsigned char *)"Trailers",
                                    8) == XQC_TRUE);
 ...
 */
void xqc_test_h3_blocked_buf_limit(void) {
  xqc_connection_t *conn = NULL;
  xqc_h3_conn_t *h3c = NULL;
  xqc_h3_stream_t *h3s = xqc_h3_msgerr_setup(&conn, &h3c);
  CU_ASSERT_FATAL(h3s != NULL);
  ...
  for (int i = 0; i < 16; i++) {
    CU_ASSERT(xqc_rep_write_indexed(fs, XQC_DTABLE_FLAG,
                                    xqc_abs2brel(base, 0)) == XQC_OK);
  }
 ...
 * RFC 9204 Section 2.1.2: decoder enforces its own
...
void xqc_test_h3_blocked_stream_limit_uses_local() {
  ...
  h3c->peer_h3_conn_settings.qpack_blocked_streams =
      XQC_H3_SETTINGS_UNSET; /* UINT64_MAX */
  ...
  CU_ASSERT(h3c->peer_h3_conn_settings.qpack_blocked_streams ==
 ...
 * ALIBABA-2026-42073004: xqc_h3_frm_parse_settings must reject frame->len >
 * 1024.
 */
...
void xqc_test_h3_field_name_uppercase_rejection() {
  ...
  CU_ASSERT(xqc_qpack_field_name_has_uppercase(single_u, sizeof(single_u) -
                                                             1) == XQC_TRUE);
  ...
  CU_ASSERT(xqc_qpack_field_name_has_uppercase(pseudo_lc, sizeof(pseudo_lc) -
                                                              1) == XQC_FALSE);
  ...
  CU_ASSERT(xqc_qpack_field_name_has_uppercase(pseudo_uc, sizeof(pseudo_uc) -
                                                              1) == XQC_TRUE);
  ...
  CU_ASSERT(xqc_qpack_field_name_has_uppercase(
                open_bracket, sizeof(open_bracket) - 1) == XQC_FALSE);
  ...
  CU_ASSERT(xqc_qpack_field_name_has_uppercase(
                trailing_uc, sizeof(trailing_uc) - 1) == XQC_TRUE);
...
void xqc_test_h3_lowercase_field_name_stream_accepted() {
  const unsigned char lowercase[] = {0x01, 0x06, 0x00, 0x00,
                                     0x21, 0x78, 0x01, 0x76};
  xqc_connection_t *conn = NULL;
  xqc_h3_conn_t *h3c = NULL;
  xqc_h3_stream_t *h3s = xqc_h3_msgerr_setup(&conn, &h3c);
...
void xqc_test_h3_uppercase_field_name_stream_rejected() {
  const unsigned char uppercase[] = {0x01, 0x06, 0x00, 0x00,
                                     0x21, 0x58, 0x01, 0x76};
  xqc_connection_t *conn = NULL;
  xqc_h3_conn_t *h3c = NULL;
  xqc_h3_stream_t *h3s = xqc_h3_msgerr_setup(&conn, &h3c);
...
void xqc_test_h3_pseudo_header_order_accepted() {
  const unsigned char pseudo_before_regular[] = {
      0x01, 0x0a, 0x00, 0x00, 0xd1, 0xd7, 0xc1, 0xc0, 0x21, 0x78, 0x01, 0x76};
  xqc_connection_t *conn = NULL;
  xqc_h3_conn_t *h3c = NULL;
  xqc_h3_stream_t *h3s = xqc_h3_msgerr_setup(&conn, &h3c);
  CU_ASSERT_FATAL(h3s != NULL);
...
void xqc_test_h3_pseudo_header_after_regular_rejected() {
  const unsigned char pseudo_after_regular[] = {
      0x01, 0x0a, 0x00, 0x00, 0xd1, 0xd7, 0xc1, 0x21, 0x78, 0x01, 0x76, 0xc0};
  xqc_connection_t *conn = NULL;
  xqc_h3_conn_t *h3c = NULL;
  xqc_h3_stream_t *h3s = xqc_h3_msgerr_setup(&conn, &h3c);
  CU_ASSERT_FATAL(h3s != NULL);
...
void xqc_test_h3_priority_bounded_parse() {
  ...
  CU_ASSERT_EQUAL(xqc_parse_http_priority(&parsed, priority, sizeof(priority)),
                  XQC_OK);
  CU_ASSERT_EQUAL(parsed.urgency, 1);
  CU_ASSERT_EQUAL(parsed.incremental, XQC_TRUE);

  CU_ASSERT_EQUAL(
      xqc_parse_http_priority(&parsed, malformed, sizeof(malformed)),
...
void xqc_test_h3_priority_queue_class() {
  ...
  CU_ASSERT_PTR_EQUAL(
      packet,
      xqc_list_entry(conn->conn_send_queue->sndq_send_packets_urgent.prev,
  ...
  CU_ASSERT_PTR_EQUAL(
      packet,
      xqc_list_entry(conn->conn_send_queue->sndq_send_packets_high_pri.prev,
                     xqc_packet_out_t, po_list));
  ...
  CU_ASSERT(
      !xqc_list_empty(&conn->conn_initial_path
