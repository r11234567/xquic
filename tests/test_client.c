 ...
 * 99 for pure fin
 * 802 for PATH_RESPONSE no-repair e2e validation
    ...
    socklen_t peer_addrlen, void *user_data)
{
    ...
    if (hsk_completed) {
        if ((g_test_case == 103 || g_test_case == 802)
    ...
    do {
        ...
        if (g_test_case == 802 && g_no_crypt && !path_response_drop_pkt1
...
xqc_client_masque_start_tunnel(user_conn_t *user_conn)
{
    ...
    /* g_host can hold 255 bytes; allow ':', a signed decimal int, and NUL. */
    char authority[sizeof(g_host) + 12];
...
int main(int argc, char *argv[]) {
    ...
    if (g_test_case == 802 && !g_no_crypt) {
        printf("test case 802 requires -N to inspect short-header frame type\n");
        exit(1);
    }
